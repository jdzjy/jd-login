...
module.exports = async (s) => {
    ...
    if (from == "system") { //由其他插件调用
		...
		if (s.param(1) == "手机号登录") {
            ...
            if (phone == "q") return s.reply(ReplyMsgs.quit);
            try {
				pin = await doLoginPhone(phone);
			} catch (e) {
				log.error(e);
				await s.reply(e);
			}
		}else if (s.param(1) == "密码登录"){
			...
			try {
				pin = await dologinPassword(account, password);
			} catch (e) {
				log.error(e);
				await s.reply(e);
			}
    ...
    async function callAPI(uri = "/", params = {}) {
		...
		log.debug("phase callAPI:"+uri+" params:" + JSON.stringify(params));
        const opt = {
            json: params,
            timeout: 10000,
        };
